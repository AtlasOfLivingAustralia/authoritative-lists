    ...
    if conservation_lists is not None:
        ...
        for state in conservation_lists:
            ...
            for i in range(len(conservation_list_urls[state])):
                conservation_list_data = pd.concat(
                    [
                        conservation_list_data,
                        lf.read_list_url(
                            url=conservation_list_urls[state][i], state=state
                        ),
                    ]
                ).reset_index(drop=True)

            # create conservation list from raw data
            conservation_list = create_conservation_list(
            ...
            conservation_list = lf.add_change_delete_list_values(
            ...
            conservation_list.to_csv(
                "data/temp-new-lists/{}".format(temp_filename), index=False
            )
            ...
            temp_filename = "{}-conservation-historical-{}.csv".format(
                state.replace(" ", "_"), datetime.now().strftime("%Y-%m-%d")
            )
            ...
            if upload:
                s3_client.upload_file(
                    Filename="data/temp-historical-lists/{}".format(temp_filename),
                    Bucket=s3_info["bucket"],
                    Key="{}/{}".format(
                        s3_info["key_conservation_historical"], temp_filename
                    ),
                )
            ...
            if not conservation_changelist.empty:
                ...
                temp_filename = "{}-conservation-changes-{}.csv".format(
                    state.replace(" ", "_"), datetime.now().strftime("%Y-%m-%d")
                )
                conservation_changelist.to_csv(
                    "data/temp-changes/{}".format(temp_filename)
                )

                # if specified, upload changes
                if upload:
                    s3_client.upload_file(
                        Filename="data/temp-changes/{}".format(temp_filename),
                        Bucket=s3_info["bucket"],
                        Key="{}/{}".format(
                            s3_info["key_conservation_changes"], temp_filename
                        ),
                    )
            ...
            if upload:
                s3_client.upload_file(
                    Filename="data/temp-new-lists/{}".format(temp_filename),
                    Bucket=s3_info["bucket"],
                    Key="{}/{}".format(
                        s3_info["key_conservation_lists"], temp_filename
                    ),
                )
    ...
    if sensitive_lists is not None:
        for state in sensitive_lists:
            ...
            for i in range(len(sensitive_list_urls[state])):
                sensitive_list_data = pd.concat(
                    [
                        sensitive_list_data,
                        lf.read_list_url(
                            url=sensitive_list_urls[state][i], state=state
                        ),
                    ]
                ).reset_index(drop=True)

            # create a processed sensitive list from the raw data
            sensitive_list = create_sensitive_list(
            ...
            oldList = lf.kvp_to_columns(oldList)
            temp_filename = "{}-sensitive-historical-{}.csv".format(
                state.replace(" ", "_"), datetime.now().strftime("%Y-%m-%d")
            )
            oldList.to_csv("data/temp-historical-lists/{}".format(temp_filename))
            ...
            if upload:
                s3_client.upload_file(
                    ...
                    Bucket=s3_info["bucket"],
                    Key="{}/{}".format(
                        s3_info["key_sensitive_historical"], temp_filename
                    ),
                )

            # generate difference report for sensitive list
            sensitive_changelist = lf.get_changelist(
                list_ids_sensitive_test[state], list_ids_sensitive_prod[state], "S"
            )
            ...
            if not sensitive_changelist.empty:
                ...
                temp_filename = "{}-sensitive-changes-{}.csv".format(
                    state.replace(" ", "_"), datetime.now().strftime("%Y-%m-%d")
                )
                sensitive_changelist.to_csv(
                    "data/temp-changes/{}".format(temp_filename), index=False
                )
                ...
                if upload:
                    s3_client.upload_file(
                        ...
                        Bucket=s3_info["bucket"],
                        Key="{}/{}".format(
                            s3_info["key_sensitive_changes"], temp_filename
                        ),
                    )
